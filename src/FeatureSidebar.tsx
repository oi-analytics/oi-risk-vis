...
const FeatureSidebar = (props) => {
  ...
  const adapt_vars_perkm = [
    "ini_adap_cost_perkm",
    "maintenance_cost_perkm",
    "tot_adap_cost_perkm",
  ];
  ...
  return (
    <div className="custom-map-control top-right selected-feature">
        ...
        <dl>
          ...
          {f.min_speed && f.max_speed ? (
            <Fragment>
              <dt>Speeds (km/hr)</dt>
              <dd>
                {f.min_speed.toFixed(0)} – {f.max_speed.toFixed(0)}
              </dd>
            </Fragment>
          ) : null}
          {f.max_tons && f.min_tons ? (
            <Fragment>
              ...
              <dd>
                {commas(f.min_tons.toFixed(0))} –{" "}
                {commas(f.max_tons.toFixed(0))}
              </dd>
            </Fragment>
          ) : null}
          {f.vehicle_co ? (
          ...
          {f.EAD_min_usd && f.EAD_max_usd ? (
            <Fragment>
              ...
              <dd>
                {f.EAD_min_usd}&mdash;{f.EAD_max_usd}
              </dd>
            </Fragment>
          ) : null}
          {f.EAEL_daily_usd ? (
            <Fragment>
              <dt>
                Expected Annual Economic Losses (per day of disruption, US$)
              </dt>
          ...
          {f.total_EAL_min_usd && f.total_EAL_max_usd ? (
            <Fragment>
              <dt>
                Total Expected Risk (EAD + EAEL for 30 day disruption, US$)
              </dt>
              <dd>
                {f.total_EAL_min_usd}–{f.total_EAL_max_usd}
              </dd>
      ...
      {f.flooding_baseline_min_height ||
      f.flooding_rcp_4p5_min_height ||
      f.flooding_rcp_8p5_min_height ||
      f.flooding_baseline_max_height ||
      f.flooding_rcp_4p5_max_height ||
      f.flooding_rcp_8p5_max_height ? (
        <details>
          ...
          <dl>
            {f.max_tr_loss && f.min_tr_loss ? (
              <Fragment>
                ...
                <dd>
                  {commas(f.min_tr_loss.toFixed(0))} –{" "}
                  {commas(f.max_tr_loss.toFixed(0))}
                </dd>
              </Fragment>
            ) : null}
            {f.max_econ_loss && f.min_econ_loss ? (
              <Fragment>
                ...
                <dd>
                  {commas(f.min_econ_loss.toFixed(0))} –{" "}
                  {commas(f.max_econ_loss.toFixed(0))}
                </dd>
              </Fragment>
            ) : null}
            {f.max_econ_impact && f.min_econ_impact ? (
              <Fragment>
                ...
                <dd>
                  {commas(f.min_econ_impact.toFixed(0))} –{" "}
                  {commas(f.max_econ_impact.toFixed(0))}
                </dd>
              </Fragment>
            ) : null}
          </dl>
      ...
      f.rcp_4p5_max_ead ||
      f.rcp_8p5_max_ead ||
      f.baseline_max_eael_per_day ||
      f.rcp_4p5_max_eael_per_day ||
      f.rcp_8p5_max_eael_per_day ? (
        <details>
                      ...
                      <tr key={scenario}>
                        ...
                        {risk_vars.map((risk_var) => {
                          ...
                          return max_risk > min_risk ? (
                            <td key={risk_var}>
                              {commas(min_risk.toFixed(0))} -{" "}
                              {commas(max_risk.toFixed(0))}
                            </td>
                          ) : (
                            <td key={risk_var}>
                              {commas(min_risk.toFixed(0))}
      ...
      {f.baseline_max_ini_adap_cost ||
      f.rcp_4p5_max_ini_adap_cost ||
      f.rcp_8p5_max_ini_adap_cost ? (
        <details>
                    ...
                    return (
                      <tr key={scenario}>
                        ...
                        {adapt_vars.map((adapt_var) => {
                          if (
                            f[scenario + "_min_" + adapt_var] ||
                            f[scenario + "_max_" + adapt_var]
                          ) {
                            const min_val = f[scenario + "_min_" + adapt_var];
                            const max_val = f[scenario + "_max_" + adapt_var];

                            return Math.floor(Math.max(min_val, max_val)) >
                              Math.floor(Math.min(min_val, max_val)) ? (
                              <td key={adapt_var}>
                                {commas(+Math.min(min_val, max_val).toFixed(0))}{" "}
                                -{" "}
                                {commas(+Math.max(min_val, max_val).toFixed(0))}
                              </td>
                            ) : (
                              <td key={adapt_var}>
                                {commas(min_val.toFixed(0))}
                              </td>
                            );
                          } else {
      ...
      {f.baseline_max_ini_adap_cost_perkm ||
      f.rcp_4p5_max_ini_adap_cost_perkm ||
      f.rcp_8p5_max_ini_adap_cost_perkm ? (
        <details>
                    ...
                    return (
                      <tr key={scenario}>
                        ...
                        {adapt_vars_perkm.map((adapt_var_perkm) => {
                          if (
                            f[scenario + "_min_" + adapt_var_perkm] ||
                            f[scenario + "_max_" + adapt_var_perkm]
                          ) {
                            const min_val =
                              f[scenario + "_min_" + adapt_var_perkm];
                            const max_val =
      ...
      {f.baseline_max_tot_adap_cost ||
      f.rcp_4p5_max_tot_adap_cost ||
      f.rcp_8p5_max_tot_adap_cost ? (
        <details>
...
class BCRWidget extends React.Component<BCRProps, BCRState, any> {
  state: BCRState;

  constructor(props: BCRProps) {
    super(props);
    this.state = {
  ...
  handleChange(e) {
    const growth_rate_percentage =
      e.target.name === "growth_rate_percentage"
        ? +e.target.value
        : this.state.growth_rate_percentage;

    const duration =
      e.target.name === "duration" ? +e.target.value : this.state.duration;

    const { discount_rate_norm, discount_rate_growth } = discount_rates(
      growth_rate_percentage / 100
    );
            ...
            {this.props.scenarios.map((scenario) => {
              ...
              return (
                <tr key={scenario}>
                  ...
                  <td>{`${commas(+data.min_benefit.toFixed(0))} – ${commas(
                    +data.max_benefit.toFixed(0)
                  )}`}</td>
                  <td>{`${data.min_bcr.toFixed(2)} – ${data.max_bcr.toFixed(
                    2
                  )}`}</td>
                </tr>
              );
...
function calculateAdaption(
  ...
  // for reference.
  const { discount_rate_norm, discount_rate_growth } =
    discount_rates(growth_rate);
  const min_benefit = benefit(
    discount_rate_norm,
    ead,
    duration,
...
function discount_rates(growth_rate) {
  ...
  const years_from_start = Array.from(Array(end_year - start_year).keys());
  ...
  const discount_rate_growths = years_from_start.map(
    (year) =>
      Math.pow(1.0 + growth_rate, year) / Math.pow(1.0 + discount_rate, year)
  );
...
function benefit(
  ...
  return (
    discount_rate_norm * ead + duration * discount_rate_growth * eael_per_day
  );
}
