...
type Region = {
  ...
  minEAD: number;
  maxEAD: number;
  minEAD_rcp4p5: number;
  maxEAD_rcp4p5: number;
  minEAD_rcp8p5: number;
  maxEAD_rcp8p5: number;
};

function RegionSummary({ region }: { region?: Region }) {
  if (!region) {
    return (
      <article>
    ...
    <article>
      ...
      <VegaEmbed
        spec={{
          ...
          data: {
            url: "historical_total/historical_total_" + r.GID_1 + ".csv",
          },
        }}
      ...
      <p>
        * Note that cyclone damages are only estimated for historical scenario.
      </p>
